...
#This program adds a small letter to every ID without letter (e.g. 170218) in order to create a unique ID (e.g. 10218c).

import os, re, string

# A function that filters filenames which end in ".png" and return them in an array "filteredFilenames". E.g. hidden files will not show up in the backup-file.
...
# This function checks, if a certain id exists in either the Zettelkasten-Directory ('../../Zettelkasten') or the directory, where new zettel get parked ('../Zettel'), until a filename is assigned to them.
def idExists(id):
...
def makeID(zettel):
	...
	for letter in string.ascii_lowercase:
		# If the combination 6 digits (e.g. '171103') plus a certain letter (e.g. 'a') does not already exist, then assign this unique ID to the filename e.g. 171103a
